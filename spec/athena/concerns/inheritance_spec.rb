...
RSpec.describe Athena::Concerns::Inheritance do
  let(:base) do
  ...
  describe ".abstract_class?" do
    it "is false by default" do
      klass = Class.new(Athena::Command) { self.command_name = :test }
  ...
  describe ".base_class" do
    ...
    it "raises if not descending from Command" do
      klass = Class.new
      klass.extend(Athena::Concerns::Inheritance::ClassMethods)
      expect { klass.base_class }.to raise_error(Athena::Error)
    end
  end
end
