...
RSpec.describe Athena::Tool do
  let(:tool_class) do
    Class.new(Athena::Tool) do
      tool_name :mytool
      version "1.0.0"
  ...
  describe "auto-created command base" do
    ...
    it "makes the command base a subclass of Athena::Command" do
      expect(tool_class::Command.superclass).to eq(Athena::Command)
    end
  ...
  describe "Tool is abstract" do
    it "does not register Tool subclasses as subcommands of Command" do
      expect(Athena::Command.subcommands).not_to have_key(:mytool)
    end
  end
end
